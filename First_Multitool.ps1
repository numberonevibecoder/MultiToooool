...
function Show-ExternalIP {
    ...
    try {
        ...
        Write-Host "  [HOST]    " -NoNewline -ForegroundColor DarkGreen; Write-Host $data.hostname  -ForegroundColor Green
        Write-Host "  [CITY]    " -NoNewline -ForegroundColor DarkGreen; Write-Host $data.city      -ForegroundColor Green
        Write-Host "  [REGION]  " -NoNewline -ForegroundColor DarkGreen; Write-Host $data.region    -ForegroundColor Green
        Write-Host "  [COUNTRY] " -NoNewline -ForegroundColor DarkGreen; Write-Host $data.country   -ForegroundColor Green
        Write-Host "  [ORG/ISP] " -NoNewline -ForegroundColor DarkGreen; Write-Host $data.org       -ForegroundColor Green
...
function Show-ActiveConnections {
    ...
    foreach ($c in $conns) {
        ...
        Write-Host ("  {0,-22} {1,-8} {2,-22} {3}" -f $c.LocalAddress, $c.LocalPort, $c.RemoteAddress, $proc) -ForegroundColor Green
    }
...
function Show-ScheduledTasksAudit {
    ...
    if ($tasks.Count -eq 0) {
        ...
        foreach ($t in $tasks) {
            ...
            Write-Host ("  {0,-40} {1,-15} {2}" -f $shortName, $t.State, $author) -ForegroundColor Green
        }
    }
...
function Show-VPNDetection {
    Write-Host "`n  [>>] Checking for active VPN / tunnel adapters..." -ForegroundColor DarkGreen
    Write-Host ""
    $vpnAdapters = Get-NetAdapter | Where-Object {
        $_.InterfaceDescription -match "VPN|TAP|TUN|WireGuard|OpenVPN|NordVPN|ExpressVPN|ProtonVPN|Mullvad|Cisco|Juniper|Pulse|GlobalProtect|Fortinet|SonicWall|L2TP|PPTP|SSTP|IKEv2" -or
        $_.Name -match "VPN|TAP|TUN|WireGuard|tun|ppp"
    }
    if ($vpnAdapters) {
        Write-Host ("  {0,-30} {1,-15} {2}" -f "NAME","STATUS","DESCRIPTION") -ForegroundColor Green
        Write-Host ("  " + "-" * 75) -ForegroundColor DarkGreen
        foreach ($v in $vpnAdapters) {
            $color = if ($v.Status -eq "Up") { "Green" } else { "Yellow" }
            $shortDesc = if ($v.InterfaceDescription.Length -gt 35) { $v.InterfaceDescription.Substring(0,34) + "..." } else { $v.InterfaceDescription }
            Write-Host ("  {0,-30} {1,-15} {2}" -f $v.Name, $v.Status, $shortDesc) -ForegroundColor $color
        }
        $activeVPN = $vpnAdapters | Where-Object { $_.Status -eq "Up" }
        if ($activeVPN) {
            Write-Host ""
            Write-Host "  +------------------------------------------+" -ForegroundColor Green
            Write-Host "  |   [ VPN ENABLED ]  Tunnel is active.    |" -ForegroundColor Green
            Write-Host "  +------------------------------------------+" -ForegroundColor Green
        } else {
            Write-Host ""
            Write-Host "  +------------------------------------------+" -ForegroundColor Red
            Write-Host "  |   [ VPN DISABLED ] Adapter not connected.|" -ForegroundColor Red
            Write-Host "  +------------------------------------------+" -ForegroundColor Red
        }
    } else {
        Write-Host ""
        Write-Host "  +------------------------------------------+" -ForegroundColor Red
        Write-Host "  |   [ VPN DISABLED ] No adapter detected.  |" -ForegroundColor Red
        Write-Host "  |   Traffic may be going out unencrypted.  |" -ForegroundColor DarkGreen
        Write-Host "  +------------------------------------------+" -ForegroundColor Red
    }
    Write-Log "VPN detection check done"
    Pause-Screen
}

function Show-USBDeviceHistory {
    Write-Host "`n  [>>] USB device history from registry..." -ForegroundColor DarkGreen
    Write-Host ""
    $usbKey = "HKLM:\SYSTEM\CurrentControlSet\Enum\USBSTOR"
    if (-not (Test-Path $usbKey)) {
        Write-Warn "USBSTOR registry key not found."
        Pause-Screen; return
    }
    $devices = Get-ChildItem $usbKey -EA SilentlyContinue
    if (-not $devices) {
        Write-OK "No USB storage devices found in history."
        Pause-Screen; return
    }
    Write-Host ("  {0,-45} {1}" -f "DEVICE","SERIAL / INSTANCE") -ForegroundColor Green
    Write-Host ("  " + "-" * 80) -ForegroundColor DarkGreen
    foreach ($dev in $devices) {
        $friendlyType = $dev.PSChildName -replace "Disk&Ven_","" -replace "&Prod_"," | " -replace "&Rev_.*",""
        $instances = Get-ChildItem $dev.PSPath -EA SilentlyContinue
        foreach ($inst in $instances) {
            $serial = $inst.PSChildName
            $shortType = if ($friendlyType.Length -gt 43) { $friendlyType.Substring(0,42) + "..." } else { $friendlyType }
            Write-Host ("  {0,-45} {1}" -f $shortType, $serial) -ForegroundColor Green
        }
    }
    Write-Host ""
    Write-Dim "Note: Only USB storage (mass storage) devices are tracked here."
    Write-Log "USB device history viewed"
    Pause-Screen
}
...
function Show-SecureBootTPM {
    Write-Host "`n  [>>] Secure Boot and TPM status..." -ForegroundColor DarkGreen
    Write-Host ""

    # Secure Boot
    Write-Host "  [-- SECURE BOOT --]" -ForegroundColor Green
    try {
        $sb = Confirm-SecureBootUEFI -EA Stop
        if ($sb) {
            Write-OK "Secure Boot: ENABLED"
        } else {
            Write-Warn "Secure Boot: DISABLED  // Firmware-level boot protection is OFF"
        }
    } catch {
        Write-Dim "Secure Boot: N/A (Legacy BIOS or unsupported)"
    }
    ...
    try {
        ...
        $tpmActiv    = $tpm.TpmActivated

        $presColor = if ($tpmPresent)  { "Green"  } else { "Red"    }
        $enabColor = if ($tpmEnabled)  { "Green"  } else { "Yellow" }
        $readColor = if ($tpmReady)    { "Green"  } else { "Yellow" }
        $actvColor = if ($tpmActiv)    { "Green"  } else { "Yellow" }

        Write-Host "  [TPM PRESENT]   " -NoNewline -ForegroundColor DarkGreen
        Write-Host $(if ($tpmPresent)  { "YES" } else { "NO"  }) -ForegroundColor $presColor
        Write-Host "  [TPM ENABLED]   " -NoNewline -ForegroundColor DarkGreen
        Write-Host $(if ($tpmEnabled)  { "YES" } else { "NO"  }) -ForegroundColor $enabColor
        Write-Host "  [TPM ACTIVATED] " -NoNewline -ForegroundColor DarkGreen
        Write-Host $(if ($tpmActiv)    { "YES" } else { "NO"  }) -ForegroundColor $actvColor
        Write-Host "  [TPM READY]     " -NoNewline -ForegroundColor DarkGreen
        Write-Host $(if ($tpmReady)    { "YES" } else { "NO"  }) -ForegroundColor $readColor

        # Try to get TPM version from WMI
        try {
            $tpmWMI = Get-CimInstance -Namespace "Root\CIMv2\Security\MicrosoftTpm" -ClassName Win32_Tpm -EA Stop
            $specVer = $tpmWMI.SpecVersion
            Write-Host "  [TPM VERSION]   " -NoNewline -ForegroundColor DarkGreen
            Write-Host $specVer -ForegroundColor Green
        } catch {}

    } catch {
        Write-Warn "Could not query TPM -- may require Admin or TPM is absent."
    }

    Write-Host ""
    Write-Dim "Secure Boot + TPM 2.0 required for Windows 11 and full disk encryption."
    Write-Log "Secure Boot / TPM check done"
    Pause-Screen
}
# ============================================================
...
function Clear-BrowserCache {
    if (-not (Confirm-Action "Clear cache folders for Chrome, Edge, and Firefox?")) { return }
    Write-Host "`n  [>>] Wiping browser caches..." -ForegroundColor DarkGreen
    Write-Host ""
    ...
    foreach ($browser in $browserPaths) {
        $totalMB = 0
        Write-Host "  [BROWSER] $($browser.Name)" -ForegroundColor Green
        if ($browser.Name -eq "Firefox") {
            # Firefox profiles need recursive cache subfolder search
            $ffBase = $browser.Paths[0]
            if (Test-Path $ffBase) {
                $cacheDirs = Get-ChildItem $ffBase -Directory -EA SilentlyContinue |
                             ForEach-Object { Get-ChildItem $_.FullName -Directory -EA SilentlyContinue } |
                             Where-Object { $_.Name -match "^cache" }
                foreach ($dir in $cacheDirs) {
                    $size = (Get-ChildItem $dir.FullName -Recurse -Force -EA SilentlyContinue | Measure-Object Length -Sum -EA SilentlyContinue).Sum
                    $mb = [Math]::Round($size / 1MB, 2)
                    $totalMB += $mb
                    Get-ChildItem $dir.FullName -Recurse -Force -EA SilentlyContinue | Remove-Item -Recurse -Force -EA SilentlyContinue
                    Write-Dim "  $($dir.FullName) (~$mb MB)"
                }
            } else {
                Write-Dim "  Firefox not found -- skipping."
            }
        } else {
            foreach ($path in $browser.Paths) {
                if (Test-Path $path) {
                    $size = (Get-ChildItem $path -Recurse -Force -EA SilentlyContinue | Measure-Object Length -Sum -EA SilentlyContinue).Sum
                    $mb = [Math]::Round($size / 1MB, 2)
                    $totalMB += $mb
                    Get-ChildItem $path -Recurse -Force -EA SilentlyContinue | Remove-Item -Recurse -Force -EA SilentlyContinue
                    Write-Dim "  $path (~$mb MB)"
                } else {
                    Write-Dim "  Not found: $path"
                }
            }
        }
        Write-OK "$($browser.Name) cache cleared  (~$totalMB MB freed)"
        Write-Host ""
    }
    Write-Warn "Close browsers before running this for best results."
    Write-Log "Browser caches cleared"
    Pause-Screen
}
...
function Show-StartupApps {
    ...
    foreach ($a in $apps) {
        ...
        Write-Host ("  {0,-30} {1,-20} {2}" -f $shortName, $a.User, $shortCmd) -ForegroundColor Green
    }
...
function Invoke-NetworkInterfaceToggle {
    if (-not (Test-AdminPrivilege)) { return }
    Write-Host "`n  [>>] Available network adapters..." -ForegroundColor DarkGreen
    Write-Host ""
    $adapters = Get-NetAdapter | Sort-Object Name
    if (-not $adapters) { Write-Warn "No network adapters found."; Pause-Screen; return }

    $i = 1
    foreach ($a in $adapters) {
        $statusColor = if ($a.Status -eq "Up") { "Green" } else { "DarkGray" }
        $statusLabel = if ($a.Status -eq "Up") { "[UP]  " } else { "[DOWN]" }
    ...
    Write-Host "  [0]  CANCEL" -ForegroundColor DarkGreen
    Write-Host ""
    $choice = Read-Host "  [?] Select adapter number"
    if ($choice -eq "0" -or $choice -eq "") { return }

    $idx = [int]$choice - 1
    if ($idx -lt 0 -or $idx -ge $adapters.Count) { Write-Err "Invalid selection."; Pause-Screen; return }

    $selected = $adapters[$idx]
    Write-Host ""
    if ($selected.Status -eq "Up") {
        if (Confirm-Action "DISABLE adapter '$($selected.Name)'?") {
            Disable-NetAdapter -Name $selected.Name -Confirm:$false -EA SilentlyContinue
            Write-OK "Adapter '$($selected.Name)' DISABLED."
            Write-Log "Adapter disabled: $($selected.Name)"
        }
    } else {
        if (Confirm-Action "ENABLE adapter '$($selected.Name)'?") {
            Enable-NetAdapter -Name $selected.Name -Confirm:$false -EA SilentlyContinue
            Write-OK "Adapter '$($selected.Name)' ENABLED."
            Write-Log "Adapter enabled: $($selected.Name)"
        }
    }
    Pause-Screen
}

function Show-MACSpoofCheck {
    Write-Host "`n  [>>] Checking for MAC address spoofing..." -ForegroundColor DarkGreen
    Write-Host ""
    Write-Host ("  {0,-28} {1,-20} {2,-20} {3}" -f "ADAPTER","PHYSICAL MAC","REPORTED MAC","STATUS") -ForegroundColor Green
    ...
    foreach ($nic in $nics) {
        $reportedMAC = $nic.MACAddress.ToUpper()

        # Pull permanent/physical MAC from registry
        $regPath = "HKLM:\SYSTEM\CurrentControlSet\Control\Class\{4D36E972-E325-11CE-BFC1-08002BE10318}"
        $physMAC = $null
        $subKeys = Get-ChildItem $regPath -EA SilentlyContinue
        foreach ($key in $subKeys) {
            $driverDesc = (Get-ItemProperty $key.PSPath -EA SilentlyContinue).DriverDesc
            if ($driverDesc -and $driverDesc -like "*$($nic.Name.Split(' ')[0])*") {
                $physMAC = (Get-ItemProperty $key.PSPath -EA SilentlyContinue).NetworkAddress
                break
            }
        }

        # Fallback: get via Get-NetAdapter which surfaces NetworkAddress (spoofed) vs physical
        $netAdap = Get-NetAdapter | Where-Object { $_.MacAddress -replace "-",":" -eq $reportedMAC } | Select-Object -First 1
        $permMAC = if ($netAdap) {
            try {
                $perm = ($netAdap | Get-NetAdapterAdvancedProperty -RegistryKeyword "NetworkAddress" -EA SilentlyContinue).RegistryValue
                if ($perm) { $perm.ToUpper() } else { $null }
            } catch { $null }
        }

        $shortName = if ($nic.Name.Length -gt 26) { $nic.Name.Substring(0,25) + "..." } else { $nic.Name }

        if ($permMAC -and ($permMAC -ne $reportedMAC)) {
            Write-Host ("  {0,-28} {1,-20} {2,-20}" -f $shortName, $permMAC, $reportedMAC) -ForegroundColor Red -NoNewline
            Write-Host " !! MAC SPOOFED !!" -ForegroundColor Red
        } elseif ($permMAC) {
            Write-Host ("  {0,-28} {1,-20} {2,-20} {3}" -f $shortName, $permMAC, $reportedMAC, "OK") -ForegroundColor Green
        } else {
            # No override found in registry -- MAC is likely hardware default
            Write-Host ("  {0,-28} {1,-20} {2,-20} {3}" -f $shortName, "(hardware default)", $reportedMAC, "OK") -ForegroundColor Green
        }
    }

    # Show spoofed banner if any were flagged
    $spoofedNics = @()
    $allNics = Get-CimInstance Win32_NetworkAdapter | Where-Object { $_.PhysicalAdapter -eq $true -and $_.MACAddress }
    foreach ($nic2 in $allNics) {
        $rMAC = $nic2.MACAddress.ToUpper()
        $na2  = Get-NetAdapter | Where-Object { $_.MacAddress -replace "-",":" -eq $rMAC } | Select-Object -First 1
        if ($na2) {
            $p2 = try { ($na2 | Get-NetAdapterAdvancedProperty -RegistryKeyword "NetworkAddress" -EA SilentlyContinue).RegistryValue } catch { $null }
            if ($p2 -and ($p2.ToUpper() -ne $rMAC)) { $spoofedNics += $nic2.Name }
        }
    }
    Write-Host ""
    if ($spoofedNics.Count -gt 0) {
        Write-Host "  +------------------------------------------+" -ForegroundColor Red
        Write-Host "  |   [ MAC SPOOFED ]  Spoofing detected!   |" -ForegroundColor Red
        Write-Host "  |   Custom MAC overrides hardware MAC.    |" -ForegroundColor DarkGreen
        Write-Host "  +------------------------------------------+" -ForegroundColor Red
    } else {
        Write-Host "  +------------------------------------------+" -ForegroundColor Green
        Write-Host "  |   [ MAC OK ]  No spoofing detected.     |" -ForegroundColor Green
        Write-Host "  +------------------------------------------+" -ForegroundColor Green
    }
    Write-Log "MAC spoof check done"
    Pause-Screen
}
# ============================================================
function Show-SystemInfo {
    ...
    Write-Host ("  {0,-18}: {1}" -f "Computer",    $cs.Name)                         -ForegroundColor Green
    Write-Host ("  {0,-18}: {1}" -f "User",        "$env:USERDOMAIN\$env:USERNAME")  -ForegroundColor Green
    Write-Host ("  {0,-18}: {1}" -f "OS",          "$($os.Caption) $($os.OSArchitecture)") -ForegroundColor Green
    Write-Host ("  {0,-18}: {1}" -f "Build",       $os.BuildNumber)                  -ForegroundColor Green
    Write-Host ("  {0,-18}: {1}" -f "Uptime (hr)", [Math]::Round(($os.LocalDateTime - $os.LastBootUpTime).TotalHours, 1)) -ForegroundColor Green
    ...
    Write-Host ("  {0,-18}: {1}" -f "Model",       $cpu.Name)                        -ForegroundColor Green
    Write-Host ("  {0,-18}: {1}" -f "Cores/Threads","$($cpu.NumberOfCores) / $($cpu.NumberOfLogicalProcessors)") -ForegroundColor Green
    Write-Host ("  {0,-18}: {1}" -f "Speed",       "$($cpu.MaxClockSpeed) MHz")      -ForegroundColor Green
    ...
    Write-Host ("  {0,-18}: {1} GB" -f "Total RAM", $totalRam) -ForegroundColor Green
    try {
    ...
    Write-Host ("  {0,-18}: {1}" -f "Manufacturer", $mb.Manufacturer)               -ForegroundColor Green
    Write-Host ("  {0,-18}: {1}" -f "Product",      $mb.Product)                    -ForegroundColor Green
    ...
    Write-Host ("  {0,-18}: {1}" -f "Manufacturer", $bios.Manufacturer)             -ForegroundColor Green
    Write-Host ("  {0,-18}: {1}" -f "Version",      $bios.SMBIOSBIOSVersion)        -ForegroundColor Green
    ...
    try {
        ...
        foreach ($disk in $disks) {
            ...
            Write-Host ("  {0,-18}: {1}  {2} GB  Serial: " -f $disk.Model, $disk.InterfaceType, $sizeGB) -ForegroundColor Green -NoNewline
    ...
    try {
        ...
        foreach ($g in $gpus) {
            ...
            Write-Host ("  {0,-18}: {1}  VRAM: {2} MB" -f "Adapter", $g.Name, $vramMB) -ForegroundColor Green
    ...
    try {
        ...
        foreach ($n in $nics) {
            Write-Host ("  {0,-18}: {1}" -f "Adapter", $n.Name)           -ForegroundColor Green
...
function Show-TopProcesses {
    ...
    foreach ($p in $procs) {
        ...
        Write-Host ("  {0,-30} {1,-10} {2,-12} {3}" -f `
            ...
            [Math]::Round($p.CPU,1)) -ForegroundColor Green
    }
...
function Show-OpSecMenu {
    $items = @(
        ...
        @{Name="VPN Detection";           Desc="Check for active VPN / tunnel adapter"},
        @{Name="USB Device History";      Desc="Registry log of all USB storage devices"}
    )
    while ($true) {
        Show-Banner
...
function Show-HardeningMenu {
    $items = @(
        ...
        @{Name="Secure Boot / TPM";      Desc="Verify firmware-level protections"}
    )
    while ($true) {
        Show-Banner
...
function Show-PrivacyMenu {
    $items = @(
        ...
        @{Name="Clear Browser Cache"; Desc="Wipe Chrome, Edge, Firefox caches"},
...
function Show-NetworkMenu {
    $items = @(
        ...
        @{Name="Interface Toggle";         Desc="Enable / disable a network adapter [ADMIN]"},
        @{Name="MAC Spoof Check";          Desc="Detect spoofed MAC addresses"}
    )
    while ($true) {
        Show-Banner
        ...
        switch ($choice) {
            ...
            "4" { Show-ARPTable }
            "5" { Invoke-NetworkInterfaceToggle }
            "6" { Show-MACSpoofCheck }
            "0" { return }
        }
    }
}
...
function Invoke-IPRepCheck {
    ...
    try {
        ...
        Write-Host "  [IP]      " -NoNewline -ForegroundColor DarkRed; Write-Host $r.ip       -ForegroundColor Green

        Write-Host "  [HOST]    " -NoNewline -ForegroundColor DarkRed; Write-Host $r.hostname  -ForegroundColor Green

        Write-Host "  [CITY]    " -NoNewline -ForegroundColor DarkRed; Write-Host $r.city      -ForegroundColor Green

        Write-Host "  [REGION]  " -NoNewline -ForegroundColor DarkRed; Write-Host $r.region    -ForegroundColor Green

        Write-Host "  [COUNTRY] " -NoNewline -ForegroundColor DarkRed; Write-Host $r.country   -ForegroundColor Green

        Write-Host "  [ORG/ISP] " -NoNewline -ForegroundColor DarkRed; Write-Host $r.org       -ForegroundColor Green

        Write-Host "  [POSTAL]  " -NoNewline -ForegroundColor DarkRed; Write-Host $r.postal    -ForegroundColor Green

        Write-Host "  [TZ]      " -NoNewline -ForegroundColor DarkRed; Write-Host $r.timezone  -ForegroundColor Green
...
function Get-WhoIsLookup {
    ...
    try {
        ...
        if ($r.domain)     { Write-Host ("  {0,-18}: {1}" -f "Domain",     $r.domain)     -ForegroundColor Green }

        if ($r.registrar)  { Write-Host ("  {0,-18}: {1}" -f "Registrar",  $r.registrar)  -ForegroundColor Green }

        if ($r.created)    { Write-Host ("  {0,-18}: {1}" -f "Created",    $r.created)    -ForegroundColor Green }

        if ($r.expires)    { Write-Host ("  {0,-18}: {1}" -f "Expires",    $r.expires)    -ForegroundColor Green }

        if ($r.nameserver) { Write-Host ("  {0,-18}: {1}" -f "Nameserver", ($r.nameserver -join ", ")) -ForegroundColor Green }
...
function Get-SuspiciousProcesses {
    ...
    if ($suspicious.Count -eq 0) {
        ...
        foreach ($s in $suspicious) {
            ...
            Write-Host ("  {0,-25} {1,-8} {2}" -f $s.Name, $s.PID, $s.Flags) -ForegroundColor Green
...
function Get-InstalledSoftwareAudit {
    ...
    foreach ($a in $apps) {
        ...
        Write-Host ("  {0,-45} {1,-15} {2}" -f $shortName, $a.DisplayVersion, $shortPub) -ForegroundColor Green
...
function Get-RecentFileActivity {
    ...
    if ($files.Count -eq 0) {
        ...
        foreach ($f in ($files | Sort-Object LastWriteTime -Descending | Select-Object -First 40)) {
            ...
            Write-Host ("  {0,-55} {1,-22} {2}" -f $short, $f.LastWriteTime.ToString("yyyy-MM-dd HH:mm:ss"), $f.SizeMB) -ForegroundColor Green
